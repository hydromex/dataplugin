...
public class PluginMain extends Plugin {
    ...
    protected int execute(File pluginDir, String ... args) {
        ...
        //optional parameters...
        final String ST_DATE = (args.length >= 3) ? args[2] : "";
        final String ED_DATE = (args.length == 4) ? args[3] : "";

        final String BASH_COMMAND = "bash";
        //bash loader script arguments
        final String GIS_DB = "griddb";
        final String RASTER_TABLE = "livneh";
        ...
        try {
            ...
            List<String> command = new ArrayList<>(Arrays.asList(PYTHON_COMMAND, scriptFiles[0].getPath(),
                    "-f", "nc", INPUT_DATA, DUMP_VAR, "-o", dumpedNCFile.getPath()));

            if (!ST_DATE.isEmpty() && !ED_DATE.isEmpty()) {
                //If those parameters given, then filter by time dimension
                //The order of the parameters should not matter...
                command.add("--stDate");
                command.add(ST_DATE);
                command.add("--edDate");
                command.add(ED_DATE);
            } //Do something similar for the spatial dimensions, if allowed

            Instant startTime = Instant.now();
            boolean ok = ProcessUtil.executeProcess(command.toArray(new String[]{}));
