...
def getLocationIndexes(ncFile, c1 = None, c2 = None, lats = None, lons = None):
    ...
    if type(ncFile) is not Dataset and type(ncFile) is not MFDataset:
        return None
...
def getPeriodIndexes(ncFile, stDate = None, edDate = None, dates = None):
    ...
    if (type(ncFile) is not Dataset and type(ncFile) is not MFDataset) or TIME_KEY not in ncFile.variables:
        return None
    ...
    #elif dates: #for list of dates some day
    else:
        #No selection of anything...
        return list(range(len(timeVar)))